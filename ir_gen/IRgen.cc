...
void AddNoReturnBlock() {
    for (auto block : llvmIR.function_block_map[now_function]) {
        ...
        if (B->Instruction_list.empty() || (!(B->Instruction_list.back()->GetOpcode()== BasicInstruction::LLVMIROpcode::RET)) && (!(B->Instruction_list.back()->GetOpcode()== BasicInstruction::LLVMIROpcode::BR_COND))) {
...
BasicInstruction::LLVMType TLLvm[7] = {
...
void Func_call::codeIR() {
    ...
    if (funcr_params != nullptr){
        ...
        for (int i = 0; i < (*params).size(); i++){
            ...
            cur_param->codeIR();
            IRgenTypeConverse(B, cur_param->attribute.T.type, cur_fparam->attribute.T.type, regnumber);//形参类型不匹配时做类型转换
