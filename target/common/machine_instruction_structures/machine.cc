...
void MachineFunction::RedirectPhiNodePredecessor(int phi_block, int old_predecessor, int new_predecessor) {
    // 在phi_block中，查找所有包含 <label old_predecessor> 的phi指令，
    // 然后将 <label old_predecessor> 替换为 <label new_predecessor>
...
void MachineDominatorTree::BuildDominatorTree(bool reverse) {
    ...
    // dom[u][v] = 1 <==> v dom u <==> v 是 dom(u) 的一部分
    // atdom[0][0] = 1;
    ...
    for (auto [u, bb] : C->block_map) {
        ...
        for (int v = 0; v <= C->max_label; v++) {
            // 如果 v 是 u 的 idom，v 必须在 dom(u) 中
            // if (atdom[u][v]) {
            if (atdom[u].getbit(v)) {
                // dom(u) = {u, ???, v, {domv path}}
                // dom(v) = {v, {domv path}}
                // ??? = NULL 集合如果 v 是 u 的 idom
